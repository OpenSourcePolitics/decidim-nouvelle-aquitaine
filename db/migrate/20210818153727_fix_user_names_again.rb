...
class FixUserNamesAgain < ActiveRecord::Migration[5.2]
  def change
    ...
    weird_characters.each do |character|
      Decidim::UserBaseEntity.where(deleted_at: nil).where("name like '%#{character}%' escape '\' OR nickname like '%#{character}%' escape '\' OR nickname like '% %' escape '\'").find_each do |entity|
